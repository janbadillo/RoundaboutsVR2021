    ...
    */
    ...
    public class VehicleAI : MonoBehaviour
    {
        ...
        void WaypointChecker(){
            ...
            {
                var systemCurrentWaypoints = trafficSystem.segments[currentTarget.segment].waypoints;
                var currentWaypointIndex = currentTarget.waypoint;
                if (currentTarget.waypoint >= systemCurrentWaypoints.Count - 1)
                {
                    var count = trafficSystem.segments[currentTarget.segment].nextSegments.Count;
                    if (count <= 0)
                    {
                        Destroy(this.gameObject); //Destroy the vehicle once it has reached the last checkpoint
                    }
                }
                //Set current target to the next
        ...
        void MoveVehicle(){
            ...
            wheelDrive.maxSpeed = trafficSystem.segments[currentTarget.segment].waypoints[currentTarget.waypoint].speed;
        ...
        private GameObject GetDetectedObstacles(out float _hitDist){
